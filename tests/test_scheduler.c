...
int main(void) {
    ...
    REQUIRE(
        mcsos_scheduler_init(&sched, &boot)
        == MCSOS_SCHED_OK);

    REQUIRE(
        mcsos_sched_validate(&sched)
        == MCSOS_SCHED_OK);
    ...
    REQUIRE(
        mcsos_sched_enqueue(&sched, &a)
        == MCSOS_SCHED_OK);

    REQUIRE(
        mcsos_sched_enqueue(&sched, &b)
        == MCSOS_SCHED_OK);

    REQUIRE(
        mcsos_sched_ready_count(&sched)
        == 2u);
